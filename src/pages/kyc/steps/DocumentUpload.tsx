...
export default function DocumentUpload({
  ...
  return (
    <div className="w-full relative group/upload">
      ...
      {!file ? (
        <label className={`relative border-2 border-dashed rounded-2xl p-4 sm:p-6 md:p-8 flex flex-col items-center justify-center transition-all duration-300 text-center overflow-hidden ${disabled ? 'opacity-50 cursor-not-allowed pointer-events-none ' : 'cursor-pointer '
          }${error
          ...
          <div className="bg-white dark:bg-slate-800 p-2.5 sm:p-3 md:p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 mb-3 group-hover/upload:scale-110 group-hover/upload:rotate-3 transition-transform duration-300">
            <FiUploadCloud className={`text-xl sm:text-2xl md:text-3xl ${error ? "text-red-500" : "text-indigo-500"}`} />
          </div>
          <div className="space-y-1">
            <p className="text-[11px] sm:text-xs md:text-sm font-semibold text-slate-700 dark:text-slate-200">
        ...
        <div className={`relative p-3 sm:p-4 backdrop-blur-sm bg-white/60 dark:bg-slate-900/60 border rounded-2xl transition-all duration-300 group/card ${error
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-start gap-3 w-full">
              ...
              <div className="relative w-14 h-14 sm:w-14 sm:h-14 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 flex-shrink-0 border border-slate-200/50 dark:border-slate-700/50 shadow-inner">
                {isPDF ? (
                  <div className="w-full h-full flex flex-col items-center justify-center bg-rose-50 dark:bg-rose-900/20">
                    <FiFileText className="text-xl sm:text-xl text-red-500" />
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs sm:text-sm text-slate-700 dark:text-slate-200 pr-2 truncate flex-1">
                    {file instanceof File ? file.name : 'Document Uploaded'}
                  </span>
                  <FiCheckCircle className="text-emerald-500 text-xs flex-shrink-0" />
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">
            ...
            <div className="flex items-center gap-2 w-full justify-end">
              <button
                ...
                className="sm:flex-none p-3 sm:p-2 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-xl hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-600 transition-all duration-200 shadow-sm inline-flex items-center justify-center gap-2 sm:gap-0"
                title="View Document"
              >
              ...
              <button
                ...
                className="sm:flex-none p-3 sm:p-2 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-xl hover:bg-rose-600 hover:text-white dark:hover:bg-rose-600 transition-all duration-200 shadow-sm inline-flex items-center justify-center gap-2 sm:gap-0"
                title="Remove File"
                type="button"
              >
      ...
      {showModal && preview && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md flex items-center justify-center z-[99999] p-4 animate-in fade-in duration-300">
